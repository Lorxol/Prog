 ...
 */
size_t array_min_index(int array[], size_t len, size_t inf)
{
	int min=array[inf], index=inf;
    	for(size_t i=inf;i<len;i++)
    	{
		if (min > array[i])
		{
			min = array[i];
...
void array_select_sort(int array[], size_t len)
{
	...
	{
		size_t j = array_min_index(array,len,i);
	//	printf("array i:%d, array j:%d  ",array[i],array[j]);
